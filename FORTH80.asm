...
;
; 				FORTH	8080
; Instruction Pointer		IP	BC
; (parameter) Stack Pointer	SP	SP
; Return stack Pointer		RP	RPP (memory)
; Working register		W	DE
;
; stack		:	|[INITS0-2] ... [SP+4] [SP+2] [SP]
...
;               |   .   |
;               |=======|
;       LIT-6 ->| 0222H |     start of dictionary
;               |   .   |
;               |   .   |
...
FIRST0	EQU	LIMIT0-BFLEN0-BFLEN0
...
UVR:	DW	0		; not used
	...
	DW	0		; PFLAG
;
; ***** INTERFACE (for CP/M-80) *****
;
; Function labels in the jump table of CP/M's BIOS
WBOOT	EQU	0	; Warm boot - reload command processor
CONSTA	EQU	3	; Console status
CONIN	EQU	6	; Console input
CONOUT	EQU	9	; Console output
LISTP	EQU	12	; Printer output
SELDSK	EQU	24	; Select disc drive
SETTRK	EQU	27	; Set track number
SETSEC	EQU	30	; Set sector number
SETDMA	EQU	33	; Set DMA address
READS	EQU	36	; Read a sector
WRITES	EQU	39	; Write a sector
;
BIOST	EQU	1	; jump table base address
;
; for calling BIOS
IOS:	LHLD	BIOST
	DAD	D
...
COUT:	DW	$+2
	...
	MVI	C,2	; select "Console output"
	CALL	0005H	; call BDOS (for using TAB)
	;
	POP	B
	JMP	NEXT
...
XSYY1:	DAD	H
	RAL
	; if CY == 1 then jump to XSYY2
	JNC	XSYY2
	;
	DAD	D	; HL <- HL + DE
	ACI	0	; A <- A + 0 + CY
XSYY2:	DCR	C	; C--
	; if C <> 0 then jump to XSYY1
	JNZ	XSYY1
	;
	RET
;
...
USLAS:	DW	$+2
	...
	POP	B
	POP	H
	; if u3 < u2 then jump to USLA1
	MOV	A,L
	SUB	C
	MOV	A,H
	SBB	B
	JC	USLA1
	; u3 = 0xFFFF, u4 = 0xFFFF. (overflow)
	LXI	H,0FFFFH	; u3
	LXI	D,0FFFFH	; u4
	JMP	USLA7
	; counter = 16
...
USLA2:	DAD	H	; HL << 1
	...
	XCHG
	DAD	H	; HL(=DE) << 1
	JNC	USLA3	; if CY(#2) == 0 then jump to USLA3
	INX	D	; DE(=HL) <- DE + CY(#2)
	ANA	A	; CY = 0 (for after "RAR")
USLA3:	XCHG
	; if CY(#1) == 0 then jump to USLA4
	RAR		; restore CY(#1) and A
...
USLA4:	MOV	A,L
	...
	SBB	B
	MOV	H,A
	; if CY == 0 then jump to USLA5
	JNC	USLA5
	; restore u3
	DAD	B	; u3 <- u3 + u2
	DCX	D	; u4--
	; u4++
USLA5:	INX	D	; u4++ only if u3 >= u2
	; if --counter >= 0 then jump to USLA2
USLA6:	POP	PSW	; restore counter
	DCR	A
	JNZ	USLA2
	; output
USLA7:	POP	B	; restore IP
	PUSH	H	; push u3
	PUSH	D	; push u4
	JMP	NEXT
;
...
CMOVE2:	MOV	A,B
	ORA	C
	JNZ	CMOVE1	; if BC <> 0 then goto CMOVE1
	POP	B	; restore IP
	JMP	NEXT
;
; ( a1 a2 n --- )
...
LCMOV2:	MOV	A,B
	ORA	C
	JNZ	LCMOV1	; if BC <> 0 then goto LCMOVE1
	POP	B	; restore IP
	JMP	NEXT
;
; ( a n b --- )
...
FILL1:	MOV	A,B
	ORA	C
	JZ	FILL2	; if BC == 0 then goto FILL1
	MOV	A,L	; A <- L
	STAX	D	; [DE] <- A
	INX	D	; DE++
	DCX	B	; BC--
	JMP	FILL1	; goto FILL1
FILL2:	POP	B	; restore IP
	JMP	NEXT
;
; ( --- ) <name>
	DB	0C1H,':'+80H
...
DOCOL:	LHLD	RPP
	...
	DCX	H
	MOV	M,C
	SHLD	RPP	; push BC to Return Stack
	INX	D	; DE++
	MOV	C,E	; BC <- DE
	MOV	B,D
	JMP	NEXT
...
DOUSE:	INX	D	; DE = PFA
	...
	MVI	D,0
	LXI	H,UP	; HL <- user variable base address
	DAD	D	; HL <- HL + DE
	JMP	HPUSH
;
; ( --- a )
...
PFIN2:	INX	H
	INX	D
	LDAX	D
	; if (A ^ M) + A <> 0 then jump to PFIN3
	XRA	M
	ADD	A
	JNZ	PFIN3	; no match
	; if CY == 0 then jump to PFIN2
	JNC	PFIN2	; A & 80H <> 0
...
DIGIT:	DW	$+2
	POP	H	; HL <- n1
	POP	D	; E <- c
	; if c < '0' then jump to DIGI2
	MOV	A,E
	SUI	30H
	JM	DIGI2
	; if c < '9' then jump to DIGI1
...
; ( --- )
	...
	DW	WORDS-7
PAREN:	DW	DOCOL
	DW	LIT,29H		; right parenthesis code
	DW	WORDS
	DW	DROP
	DW	SEMIS
;
...
EXPEC2:	DW	DUPE
	DW	LIT,0DH		;   carriage return code
	DW	EQUAL
	DW	ZBRAN,EXPEC4-$	;   IF
	DW	LEAVE
	DW	DROP
...
UPDAT:	DW	DOCOL
	...
	DW	ATT
	DW	ATT
	DW	LIT,8000H	; Set most significant bit.
	DW	ORR
	DW	PREV
	DW	ATT
	DW	STORE
...
DRONE:	DW	DOCOL
	...
	DW	SEMIS
;
; ( a n f --- ; read/write disk, read if f=1, write if f=0 )
; a: buffer address
; n: block number
; f: direction flag
	DB	83H,'R/','W'+80H
